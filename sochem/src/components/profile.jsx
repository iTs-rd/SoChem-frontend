...
function Profile(){
    ...
    useEffect(()=>{
        fetch('https://api.sochem.in/api/user-from-token/', {
            method: 'POST',
            headers: {
    ...
    const getUserDetail = () =>{
        if(userDetail==null && user){
            fetch(`https://api.sochem.in/api/user-extension?id=${user.id}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Token ${token['mr-token']}`,
    ...
    const getAllForum = () =>{
        if(user && (gotAllForm == false)){
            gotAllForm = true;
            fetch(`https://api.sochem.in/api/forum-post?id=${user.id}`, {
                method: 'GET',
                headers: {
                  'Authorization': `Token ${token['mr-token']}`
                }
    ...
    const getCommentCount = () =>{
        if(user && (gotCommentCount==false)){
            ...
            fetch(`https://api.sochem.in/api/total-comments?user_id=${user.id}`, {
                method: 'GET',
                headers: {
                  'Authorization': `Token ${token['mr-token']}`
                }
    ...
    const getAllComment = () =>{
        if(user && (gotAllComment==false)){
            gotAllComment = true;
            fetch(`https://api.sochem.in/api/forum-comment?user_id=${user.id}`, {
                method: 'GET',
                headers: {
                  'Authorization': `Token ${token['mr-token']}`
                }
